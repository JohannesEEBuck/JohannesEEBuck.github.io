require.config({
    paths: {
        ...
        'datepicker-de': 'jquery/jquery.ui.datepicker-de',
        'datepicker-fr': 'jquery/jquery.ui.datepicker-fr',
        'monthPicker': 'jquery/MonthPicker',
        'jquery_maskedinput': 'jquery/jquery.maskedinput',
        timer: 'timer',
        'bcc_commons': 'bcc_commons',
        'mapael': 'mapael/jquery.mapael',
        'mapael_eu': 'mapael/european_union',
        'mapael_mt': 'mapael/malta',
        'Raphael': 'mapael/raphael',
...
require(['jquery',
    ...
    'datepicker-fr',
    'bcc_map'],
    function ($, dataSetLoader, i18nextBCC) {
        i18nextBCC.onChangedFunction = function(lng){
            $.datepicker.setDefaults($.datepicker.regional[lng]);
            $(document.body).localize();
        }
        i18nextBCC.initI18Next(false);
        dataSetLoader.onready = function (dataSet, xml, xhttp, file){
            var dataSetInfo = initDataSetInfo(dataSet);
            bccMapApp = new BCCMapApp(dataSet, dataSetInfo, '#vmap_map', '#monthpicker', '#current_month', '#ea_clock', '#mapPlayer');
        }
