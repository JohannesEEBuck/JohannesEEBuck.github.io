(function (window) {
    ...
    c3_chart_fn.focus = function (targetIds, noDefocusOthers) {
    ...
    c3_chart_fn.select = function (ids, indices, resetOther, force) { // moded add force param to force selection even if data.selection.enabled=false
        var $$ = this.internal, d3 = $$.d3, config = $$.config;
        if (! config.data_selection_enabled && !force) { return; }
        $$.main.selectAll('.' + CLASS.shapes).selectAll('.' + CLASS.shape).each(function (d, i) {
            ...
            if (isTargetId && isTargetIndex) {
                if (config.data_selection_isselectable(d) && !isSelected && shape.style("opacity")!=0) { // modded to select only visible points
        ...
        if(resetOther && (config.data_selection_grouped || !ids) && !!indices && indices.length>0){ // moded to remove persistent selectedCircles for points which have not the class 'selected' anymore
            $$.main.selectAll('.' + CLASS.selectedCircle).each(function (d, index) {
                var selectedCircle = d3.select(this);
                var mustKeep = false;
                for(var i=0; i<indices.length; i++){
                    mustKeep = mustKeep || selectedCircle.classed(CLASS.selectedCircle+"-"+indices[i]);
                }
                if(!mustKeep){
                    selectedCircle.transition().duration(100).attr('r', 0).remove();
                }
            });
        }
    };
    c3_chart_fn.unselect = function (ids, indices, force) { // moded add force param to force unselection even if data.selection.enabled=false
        var $$ = this.internal, d3 = $$.d3, config = $$.config;
        if (! config.data_selection_enabled && !force) { return; }
        $$.main.selectAll('.' + CLASS.shapes).selectAll('.' + CLASS.shape).each(function (d, i) {
            var shape = d3.select(this), id = d.data ? d.data.id : d.id,
