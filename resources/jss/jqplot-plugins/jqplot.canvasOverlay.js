...
(function($) {
    ...
    function LineBase() {
        ...
        this.options = {           
            ...
            // prop: label
            // Text to include in a label for the line.
            label: null,
            // prop: showLabel
            // Show a label.
            showLabel: false,
            // prop: labelLocation
            // Where to position the label, 'n', 'ne', 'e', 'se', 's', 'sw', 'w', 'nw'
            labelLocation: 'nw',
            // prop: labelOffset
            // Pixel offset of label from the line.
            labelOffset: 4,
            // prop: labelAnchor
            // The end of the line to position the label near, 'start', 'end'
            labelAnchor: 'end',
            // prop: labelOffsetX
            // Additional pixel offset on axe X of label from the line
            labelOffsetX: 0,
            // prop: labelOffsetY
            // Additional pixel offset on axe Y of label from the line
            labelOffsetY: 0
        };
    }
        ...
        if (this.options.show) {
            ...
            for (var k=0; k<objs.length; k++) {
                ...
                if (obj.options.show) {
                    ...
                    if (obj.options.showLabel) {
                    	createLabel(plot, obj);
                    }
                }
            }
        }
    ...
    function createLabel(plot, line) {
    	var anchor, originalHeight, element = $('<div/>');
    	element.addClass('jqplot-canvasOverlay-label');
    	element.css({position: 'absolute', display: 'none'});
    	element.text(line.options.label);
    	element.appendTo(plot.targetId);
    	if (line.options.labelAnchor === 'start') {
    		anchor = line.gridStart;
    	} else {
    		anchor = line.gridStop;
    	}
    	originalHeight = element.outerHeight(true);
    	positionLabel(element, line, anchor, plot);
    	// Recalculate the appropriate position if wrapping was triggered.
    	if (originalHeight !== element.outerHeight(true)) {
    		positionLabel(element, line, anchor, plot);
    	}
    		element.css('display', 'block');
    	}
    	function positionLabel(element, line, anchor, plot) {
    	var position = calculatePosition(element, line.options.labelLocation, line.options.labelOffset, anchor, plot);
    	element.css('left', position.x+line.options.labelOffsetX);
    	element.css('top', position.y+line.options.labelOffsetY);
	}
    ...
    function showTooltip(plot, obj, gridpos, datapos) {
        ...
        var opts = obj.options, positions;

        elem.html($.jqplot.sprintf(opts.tooltipFormatString, datapos[0], datapos[1]));
        
        positions = calculatePosition(elem, opts.tooltipLocation, opts.tooltipOffset, gridpos, plot);

        elem.css('left', positions.x);
        elem.css('top', positions.y);
        if (opts.fadeTooltip) {
    ...
    function calculatePosition(element, location, offset, gridPosition, plot) {
    	var x, y;
    	switch (location) {
	    	case 'nw':
		    	x = gridPosition[0] + plot._gridPadding.left - element.outerWidth(true) - offset;
		    	y = gridPosition[1] + plot._gridPadding.top - offset - element.outerHeight(true);
		    	break;
	    	case 'n':
		    	x = gridPosition[0] + plot._gridPadding.left - element.outerWidth(true) / 2;
		    	y = gridPosition[1] + plot._gridPadding.top - offset - element.outerHeight(true);
		    	break;
	    	case 'ne':
		    	x = gridPosition[0] + plot._gridPadding.left + offset;
		    	y = gridPosition[1] + plot._gridPadding.top - offset - element.outerHeight(true);
		    	break;
	    	case 'e':
		    	x = gridPosition[0] + plot._gridPadding.left + offset;
		    	y = gridPosition[1] + plot._gridPadding.top - element.outerHeight(true) / 2;
		    	break;
	    	case 'se':
		    	x = gridPosition[0] + plot._gridPadding.left + offset;
		    	y = gridPosition[1] + plot._gridPadding.top + offset;
		    	break;
	    	case 's':
		    	x = gridPosition[0] + plot._gridPadding.left - element.outerWidth(true) / 2;
		    	y = gridPosition[1] + plot._gridPadding.top + offset;
